...
public:
  ...
  */
  virtual bool getCollidingTriangles(int nTri, float t1[9], float t2[9], bool ModelSpace=true) = 0;
  ...
  */
  ...
  /** Retrieve the detected collision point. 
      ...
      be transformed by the model's current transform to world space.
  */
  virtual bool getCollisionPoint(int nPnt, float p[3], bool ModelSpace=true) = 0;

  virtual int  getNumCollisions(void) = 0;
};
